...
passport.use(new GoogleStrategy({
    ...
    try {
        ...
        const accessToken = await generateJwtToken({ fullname: user?.fullname, email: user?.email, image: user?.image }, process.env.ACCESS_TOKEN_SECRET_KEY, 1);
        const refreshToken = await generateJwtToken({ fullname: user?.fullname, email: user?.email, image: user?.image }, process.env.REFRESH_TOKEN_SECRET_KEY, 7);
